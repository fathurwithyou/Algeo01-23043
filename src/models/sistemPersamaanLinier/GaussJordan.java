...
public class GaussJordan {
    ...
    public Matrix gaussJordanElimination(Integer n, Integer m, Matrix augmentedMatrix) {
        int rows = n;
        int cols = m;
        boolean[] isPivotColumn = new boolean[cols];
        ...
        for (int col = 0; col < cols; col++) {
            ...
            double pivotElement = augmentedMatrix.get(pivotRow, col);
            for (int j = 0; j < augmentedMatrix.getColumnCount(); j++) {
                augmentedMatrix.set(pivotRow, j, augmentedMatrix.get(pivotRow, j) / pivotElement);
            }
    ...
    public Array main(Matrix data) {
        ...
        int m = data.getColumnCount() - 1;
        Matrix result = gaussJordanElimination(n, m, data);
        Array solution = new BackwardSubstitution().backwardSubstitution(result);
        return solution;
    }
}
